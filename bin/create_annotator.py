...
def get_args():
    ...
    parser.add_argument(
        "-a",
        "--assembly",
        help="The name of the assembly to pre-load annotation (so, mm10 for the 10th mouse assembly)",
    )
...
def get_exon_number(row):
    if "gene_name" in row["attribute"]:
        return int(
            row["attribute"].split("exon_number")[-1].split(";")[0].split('"')[1]
        )
            ...
            for i in range(1, max(group2["exon_number"])):
                splices[name1].add(
                    tuple(
                        sorted(
                            [
                                group2[group2["exon_number"] == i].iloc[0]["end"],
                                group2[group2["exon_number"] == i + 1].iloc[0]["start"],
                            ]
                        )
                    )
                )
...
def main():
    ...
    annotator_path = "{}_gene_names.pkl".format(wrapper_path, args.assembly)
    ...
    if save_exon_bounds:
        ...
        print("{}_exon_bounds.pkl".format(wrapper_path, args.assembly))

    if save_splices:
        splices = get_splices(gtf_df)
        pickle.dump(
            splices,
            open("{}_splices.pkl".format(wrapper_path, args.assembly), "wb"),
        )

        print("{}_splices.pkl".format(wrapper_path, args.assembly))
