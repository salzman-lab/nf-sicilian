...
class Annotator:
    ...
    def get_gtf_dict(self):
        ...
        for seqname in gtf_df["seqname"].unique():
            ...
            if seqname_len_dict[seqname] < max(
                gtf_df[gtf_df["seqname"] == seqname]["start"]
            ):
        ...
        gtf_dict = {
            s: {r: {} for r in range(0, seqname_len_dict[s], self.jump)}
            for s in seqname_len_dict.keys()
        }
        ...
        for seqname in seqname_len_dict:
            ...
            for gene_id in seqname_df["gene_id"].unique():
                if gene_id is not None:
                    ...
                    for j in range(
                        round_down(start, self.jump),
                        round_down(end + self.jump, self.jump),
                        self.jump,
                    ):
                        gtf_dict[seqname][j][gene_id] = [start, end, strand]
        self.gtf_dict = gtf_dict

    def get_name_given_locus(
        self, seqname, position, read_strand="", stranded_library=False
    ):
