# nf-core/sicilian: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/sicilian/usage](https://nf-co.re/sicilian/usage)
...
```bash
nextflow run nf-core/sicilian --input '*_R{1,2}.fastq.gz' -profile docker
```
...
```bash
nextflow pull nf-core/sicilian
```
...
First, go to the [nf-core/sicilian releases page](https://github.com/nf-core/sicilian/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/sicilian`](https://hub.docker.com/r/nfcore/sicilian/)
* `singularity`
